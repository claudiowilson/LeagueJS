...
class ApiRequest {
	 ...
	 * @param {number} numRetriesLeft amount of retries left to do on bugged 429 or 503
	 * @param {number} intervalRetryMS tim to wait between retries on bugges 429  or 503
	 */
	static executing(url, {token, resolveWithFullResponse = false, numRetriesLeft = 3, intervalRetryMS = 500} = {}) {
		if (!url) {
		...
		return requestP(options).catch(err => {
			...
			let is429Bug = err.statusCode === 429 && (!err.response.headers['retry-after']);
			if (numRetriesLeft > 0 && (is429Bug || err.statusCode === 503)) {
				numRetriesLeft--;
				return Bluebird
