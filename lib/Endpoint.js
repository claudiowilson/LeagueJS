...
class Endpoint {
	...
	executingRequest(endpointUrl, platformIdOrRegion = this.config.PLATFORM_ID, options = {}) {
		return Bluebird.resolve()
			.then(() => {
				...
				if (this.rateLimiter) {
					return this.executingRequestWithRateLimit(endpointUrl, platformId, options);
				} else {
					return this.executingRequestWithoutRateLimit(endpointUrl, platformId, options);
				}
			});
	}
	 ...
	 * @see {@link Endpoint.executingRequestWithoutRateLimit}
	 */
	executingRequestWithRateLimit(endpointUrl, platformIdOrRegion = this.config.PLATFORM_ID, options = {}) {
		return Bluebird.resolve()
			.then(() => {
				const platformId = RegionAndPlatformUtil.getPlatformIdFromPlatformIdOrRegion(platformIdOrRegion);
				...
				if (cacheValue) {
					...
					return this.rateLimiter.scheduling(platformId, () => ApiRequest.executing(requestUrl, this.requestOptions))
						.then(response => {
	 ...
	 * @see {@link Endpoint.executingRequestWithRateLimit}
	 */
	executingRequestWithoutRateLimit(endpointUrl, platformIdOrRegion = this.config.PLATFORM_ID, options = {}) {
		return Bluebird.resolve()
			.then(() => {
				const platformId = RegionAndPlatformUtil.getPlatformIdFromPlatformIdOrRegion(platformIdOrRegion);
