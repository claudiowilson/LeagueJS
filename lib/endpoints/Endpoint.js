...
class Endpoint {
	...
	constructor(name, config, rateLimiter) {
		if (rateLimiter){
			EndpointUtil.throwIfNotRateLimiter(rateLimiter, 'rateLimiter');
		}
	 ...
	 * @see {@link Endpoint.executingRequestWithoutRateLimit}
	 */
	executingRequestWithRateLimit(endpointUrl, platformId = this.config.PLATFORM_ID, query = '') {
	...
	executingRequestWithoutRateLimit(endpointUrl, platformId = this.config.PLATFORM_ID, query = '') {
		...
		if (cacheValue) {
			...
			return ApiRequest.executing(requestUrl, {token: this.config.API_KEY})
				.then(response => {
					if (this.cache) {this.cache.set(requestUrl, response);}
					return response;
				});
