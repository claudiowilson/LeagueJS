...
class Endpoint {
	...
	constructor(name, config, platformIds) {
		...
		platformIds.forEach(platformId => {
			this.rateLimiter10[platformId] =
				new RateLimiter(this.config.limits.per10, 10 * 1000, this.config.limits.allowBursts);
			this.rateLimiter600[platformId] =
				new RateLimiter(this.config.limits.per600, 600 * 1000, this.config.limits.allowBursts);
		});
	}
	...
	/**
	 * Execute a APIRequest based on the Config of this Endpoint without using the rate limiter
	 * if there is a Cached version of the request it will be used instead of making a request against the server
	 */
	executingRequestWithoutRateLimit(endpointUrl, platformId = this.config.PLATFORM_ID, query = '') {
		// TODO: add configurable retries on 503 - {maxRetries: number, retryIntervalMS: number, retryEndpoints: Array}
		...
		if (cacheValue) {
			...
			// TODO: rate-limiting should be disabled for Static-data endpoint
			// TODO: rework ratelimit scheduling - 1:1 adaption from previous handling
			return ApiRequest.executing(requestUrl, {token: this.config.API_KEY}).then(response => {
				if (this.cache) {this.cache.set(requestUrl, response);}
				return response;
			});
		}
