...
class ChampionEndpoint extends Endpoint {
	...
	gettingList(platformIdOrRegion, {freeToPlay = false} = {}) {
		const {_platformId, _options} =
			ParameterUtil.extractPlatformIdAndOptions(platformIdOrRegion,{freeToPlay});
		ErrorUtil.throwIfNotBoolean(freeToPlay, 'freeToPlay');
		return this.executingRequest(``, _platformId, EndpointUtil.buildQueryStringFromOptions(_options)); // TODO: move into Endpoint class (executing...)
	}
