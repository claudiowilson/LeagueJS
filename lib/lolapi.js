...
(function () {
    ...
    var League = {},
        ...
        observerUrl = 'api.pvp.net/observer-mode/rest/consumer',
    ...
    League.getPlatformId = function (region, callback) {
        ...
        if(platformId) {
            callback(null, platformId);
        } else {
            callback('Invalid region', null);
        }
    }
    ...
    League.getShards = function(callback) {
        ...
        util.makeRequest(url, 'Error getting shards: ', null, callback);
    };
    ...
    League.getShardByRegion = function(regionOrFunction, callback) {
        ...
        util.makeRequest(url, 'Error getting shards by region ', null, callback);
    };
    ...
    League.getMatch = function(matchId, includeTimelineOrFunction, regionOrFunction, callback) {
        ...
        util.makeRequest(url, 'Error getting match: ', null, regionAndFunc.callback);
    };
    ...
    League.getMatchHistory = function(summonerId, options, regionOrFunction, callback) {
        ...
        util.makeRequest(url, 'Error getting match history: ', null, regionAndFunc.callback);
    };
    League.getCurrentGame = function(summonerId, regionOrFunction, callback) {
        var currentGameUrl = '/getSpectatorGameInfo',
            url,
            regionAndFunc = util.getCallbackAndRegion(regionOrFunction, region, callback),
            platformId;
        League.getPlatformId(regionAndFunc.region, function(err, platform){
            platformId = platform;
        });
        url = util.craftObserverUrl(observerUrl, regionAndFunc.region, currentGameUrl + '/' +
            platformId + '/' + summonerId + '?', authKey);
        util.makeRequest(url, 'Error getting current game: ', null, regionAndFunc.callback);
    };
